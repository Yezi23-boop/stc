...
void qprFxpInit(tqprControllerFxp* qpr,
                Frac16_t kr, Frac16_t wc_inc, Frac16_t w0_inc,uint16_t sample_freq,
                Frac16_t y_limit);

// 运行：errorK 为 Q15，newW0_inc 为新的 w0 离散角增量（Q15）；若与当前不同将实时更新系数
// 返回 yK（Q15）
Frac16_t qprFxpUpdate(tqprControllerFxp* qpr, Frac16_t errorK, Frac16_t newW0_inc,uint16_t sample_freq);
