...
void FMSTR_ProcessSCI(void)
{
    ...
    if(TxEnableFlag)//if (pcm_wFlags.flg.bTxActive)
    {
        /* able to accept another character? */
        if(UART1->ISR_b.TBSYIF)//Wait for sending idle if(!FMSTR_SCI_TXFULL(wSciSR))
        {
            FMSTR_U8 ch;
            /* just put the byte into the SCI transmit buffer */
            if(!FMSTR_Tx(&ch))
                UART_DataTransmit((FMSTR_U8) ch);
        }
        ...
        if(UART1->ISR_b.RXCIF)// receive complete? if(FMSTR_SCI_RXREADY(wSciSR))
        {
            /*lint -esym(550, nRxChar) */
            volatile FMSTR_U16 nRxChar;
            nRxChar = UART_DataReceive(); //nRxChar = FMSTR_SCI_GETCHAR(); 
    ...
    {
        /* data byte received? */
        if(UART1->ISR_b.RXCIF)// if (FMSTR_SCI_RXREADY(wSciSR))
        {
            FMSTR_BCHR nRxChar = 0U;
            nRxChar = UART_DataReceive();	 //nRxChar = (FMSTR_BCHR) FMSTR_SCI_GETCHAR();
