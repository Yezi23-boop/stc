...
MotorConfig_t const MotorInit = {PWMB_CH2_P13, IO_P14, PWMB_CH3_P52, IO_P53, 17000, 40, 40, 17000, 17000};
MotorDriver g_motor_driver = {hal_motor_Init, motor_output};
EncoderDriver g_encoder_driver = {encoder_dir_init, encoder_get_count, encoder_clear_count, 0};
static struct
{
    ...
    u32 system_tick;        // 系统滴答计数
    u32 last_stats_time;    // 上次统计时间

    u8 is_running;               // 调度器运行状态
...
static TaskDescriptor_t *find_highest_priority_ready_task(void)
{
    ...
    for (i = 0; i < scheduler_private.task_count; i++)
    {
        ...
        if (task->state == TASK_READY)
        {
            ...
            u32 time_since_last_run = current_time - task->last_run_time;
...
static void check_task_deadlines(void)
{
    ...
    for (i = 0; i < scheduler_private.task_count; i++)
    {
        ...
        {
            u32 run_time = current_time - task->last_run_time;
...
static void scheduler_init(void)
{
    ...
    scheduler_private.is_running = ZF_FALSE;
                                               ...
                                               const char *name) reentrant
{
    ...
    static u8 pool_index = 0;
    TaskDescriptor_t *task;
...
// 添加任务到调度器
static u8 scheduler_add_task(TaskDescriptor_t *task)
{
    if (task == NULL)
    {
...
static void scheduler_start(void)
{
    ...
    scheduler_private.is_running = ZF_TRUE;
    scheduler_private.system_tick = 0;
...
// 获取系统滴答计数
static u32 scheduler_get_tick_count(void)
{
    return scheduler_private.system_tick;
}
...
static void scheduler_run(void)
{
    ...
    // 1. 查找并执行就绪的最高优先级任务
    task_to_run = find_highest_priority_ready_task();
...
void task_state_machine(void *context)
{
    ...
    current_time = scheduler_get_tick_count();
...
void task_system_monitor(void *context)
{
    ...
    // 监控CPU使用率
    stats = g_task_scheduler.get_stats();
...
void motor_output(int32 lpwm, int32 rpwm)
{
    ...
    // 应用死区
    if (lpwm > 0 && lpwm < private->deadband_left)
        lpwm = private->deadband_left;
    if (lpwm < 0 && lpwm > -private->deadband_left)
        lpwm = -private->deadband_left;
    if (rpwm > 0 && rpwm < private->deadband_right)
        rpwm = private->deadband_right;
    if (rpwm < 0 && rpwm > -private->deadband_right)
        rpwm = -private->deadband_right;
