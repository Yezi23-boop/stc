...
void state_machine_process_event(CarEvent_t event)
{
    ...
    for (uint8_t i = 0; i < sizeof(state_transitions)/sizeof(state_transitions[0]); i++) {
        if (state_transitions[i].from_state == g_state_machine.current_state &&
            ...
            g_state_machine.state_timeout = state_transitions[i].timeout;
...
{
    return state_names[g_state_machine.current_state];
}
// 关闭外事的输出，清零标志位
void state_idle(void)
{
	...
	// 清理编码器
	g_encoder_driver.clear();
	// 蜂鸣器

	//其他外设的输出

    //PID积分清零
    if (KEY_START_MOTOR) {                      // 比如：接收到上位机启动指令、检测到启动按键按下等
        state_machine_process_event(STATE_IDLE);  // 触发事件，而非直接赋值
    }
...
#define YAWTIMEOUT 5000  //5S
    ...
    switch (search_step) {
        case 0: hal_motor_set_pwm(200, -200); break;  // 左转
        case 1: hal_motor_set_pwm(-200, 200); break;  // 右转
        case 2: hal_motor_set_pwm(150, 150); break;   // 前进
        case 3: hal_motor_set_pwm(-150, -150); break; // 后退
    }
    // 检查是否找到线路
    if (0 == lost_spto) {
